...
export default function CicloDeVida({ initialCount }: CicloDeVidaProps) {
	...
	useEffect(() => {
			...
			if (event instanceof CustomEvent && event.detail) {
				switch (event.type) {
					case 'onCounterMount':
						...
						break;
					case 'onCounterUnmount':
						console.log('Counter desmontado');
						break;
					case 'onCounterUpdate':
						console.log('Counter atualizado para:', event.detail.count);
						break;
					default:
						break;
				};
		...
		window.addEventListener('onCounterMount', eventHandler);
		window.addEventListener('onCounterUnmount', eventHandler);
		window.addEventListener('onCounterUpdate', eventHandler);

		return () => {
			window.removeEventListener('onCounterMount', eventHandler);
			window.removeEventListener('onCounterUnmount', eventHandler);
			window.removeEventListener('onCounterUpdate', eventHandler);
		};
	}, []);
